 ...
 *
 * todo [OPEN]:
 *  - get rid of this really bad workaround to get the first response working --> $@BAD_WORKAROUND
 *
...
import { isString }        from "/evolux.util/index.mjs";
import { doAsync }         from "/evolux.universe/lib/thoregonhelper.mjs";
...
export default class Queue extends QueueBase {
    ...
    async consumer(root, responseHandler) {
        ...
        await doAsync();
/*  should be here w/o $@BAD_WORKAROUND
...
*/
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.root.get(REQUESTQ).get(reqid).put(eentry, (ack) => {
                    ...
                    this.initResponse();    // $@BAD_WORKAROUND
                });
                await doAsync();
                resolve();
            } catch (e) {
    ...
    async handleResponse(item, key) {
        if (item == undefined) return;
        this.responseInitialized();     // $@BAD_WORKAROUND
        await doAsync();
        const { encrypt, decrypt } = await this.getCrypto();
        const sentry = JSON.parse(item.substr(2));
