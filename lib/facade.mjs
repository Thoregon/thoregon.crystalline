...
const SERVICE_TIMEOUT = globalThis.thoregon?.isDev ? 5000000 : 60000;   // milliseconds
...
export default class Facade {
        ...
        Object.entries(schema.events).forEach(([name, spec]) => {
            ...
            if (!Reflect.has(this, hookname)) {
                items[hookname] = {
                    configurable: false,
                    enumerable: true,
                    set: (handler) => this.subscribe(name, handler, args)
                }
            } else {
    ...
    async subscribe(name, handler, args) {
        await this._request({ req: 'subscribe', name }, () => this.$provider.subscribe(name, (evt) => this._emit(evt, handler), args));
        return this;
    }
    ...
    describe() {
        let description = `Facade for: ${this.$provider.describe()}`;
        const props = Object.getOwnPropertyNames(this);
        props.forEach(property => description += `\nprop: ${property.name})`);
        return description;
    }
}
