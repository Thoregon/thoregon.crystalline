...
export default class RemoteConsumer extends ServiceConsumer {
    ...
    /*async*/
    remoteRequest(msg, handler) {
        return new Promise((resolve, reject) => {
            const cmd = msg.cmd;
            if (cmd === 'subscribe') {
                if (!handler) resolve();
                const subname = msg.name;
                this._subscriptions[subname] = async (evt) => {
                    await handler(evt);
                    await resolve();
                }
            } else {
    ...
    async subscribe(name, handler) {
        return await this.remoteRequest({ cmd: 'subscribe', name }, handler);
    }

    async unsubscribe(name, handler) {
        if (!handler) return;
        return await this.remoteRequest({ cmd: 'unsubscribe', name }, handler);
    }
