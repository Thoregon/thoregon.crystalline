...
import { timeout }                                              from '/evolux.universe/common.mjs';
...
export default class RemoteObserver extends AccessObserver {
    ...
    _availableRemotePropsIncludes(prop) {
        const a = [...(this.accessorNames ?? []), ...['created', 'modified']];
        // return [...(this.accessorNames ?? []), ...['created', 'modified']].includes(prop);
        return a.includes(prop);
    }

    doGet(target, prop, receiver, opt = {}) {
        if (this._availableRemotePropsIncludes(prop)) {
            const { proxy, chain } = PromiseChain.with(async (resolve, reject) => {
                try {
                    const srv = await this.__getRemoteService__();
    ...
    doSet(target, prop, value, receiver, opt = {}) {
        ...
        if (this.refs?.[prop]) {
            ...
            (async () => {
                const srv = await this.__getRemoteService__();
                srv.setEntityProperty(this.soul, prop, this.doAdjustValue(value));
            })()
        }
        return true;
    }
